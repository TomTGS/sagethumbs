...
#define LOADDLL_H
/*
 * The source code presented in this file is the "non-final" product of my on-going 
 * attempt to seek the _ideal_ dll function loader pattern. 
 * The current implementation is targeted for MSVC6. 
 * -DKLT 2003
 */
	...
	//------------------------
		...
		if (!dll) {
			...
			if (hInstance) {
				...
				ATLTRACE( "Trying LoadLibrary (%s)\n", (LPCSTR)CT2A( self + dllName ) );
				dll = ::LoadLibrary (self + dllName);
			}
			if (!dll) {
				ATLTRACE( "ERROR: cannot load library (%s)\n", (LPCSTR)CT2A( dllName ) );
				return false;
			}
		}
		return true;
